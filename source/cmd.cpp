...
static std::string encode_utf8_unit(u32 codePoint) {
    std::string out = "";
    if (codePoint < 0x000080) {
        ...
        out += (char)(0x80 | (codePoint & 63));
#ifndef RFC2279
    } else if (codePoint < 0x110000) { // RFC 3629
        out += (char)(0xF0 | (codePoint >> 18));
        out += (char)(0x80 | (codePoint >> 12 & 63));
        out += (char)(0x80 | (codePoint >> 6 & 63));
        out += (char)(0x80 | (codePoint & 63));
#else
    } else if (codePoint < 0x200000) { // RFC 2279
        out += (char)(0xF0 | (codePoint >> 18));
        out += (char)(0x80 | (codePoint >> 12 & 63));
        out += (char)(0x80 | (codePoint >> 6 & 63));
        out += (char)(0x80 | (codePoint & 63));
    } else if (codePoint < 0x4000000) { // RFC 2279
        out += (char)(0xF8 | (codePoint >> 24));
        out += (char)(0x80 | (codePoint >> 18 & 63));
        out += (char)(0x80 | (codePoint >> 12 & 63));
        out += (char)(0x80 | (codePoint >> 6 & 63));
        out += (char)(0x80 | (codePoint & 63));
    } else if (codePoint < 0x80000000) { // RFC 2279
        out += (char)(0xFC | (codePoint >> 30));
        out += (char)(0x80 | (codePoint >> 24 & 63));
        out += (char)(0x80 | (codePoint >> 18 & 63));
        out += (char)(0x80 | (codePoint >> 12 & 63));
        out += (char)(0x80 | (codePoint >> 6 & 63));
        out += (char)(0x80 | (codePoint & 63));
        ...
        printf("fromhex(): Unsupported codepoint: U+%08X\n", codePoint);

    return out;
}

static u64 fromhex(const std::string& str, size_t off = 0, size_t size = -1UL, bool optionalHead = false) {
    std::string hexMap =
        "0123456789ABCDEF"
        "0123456789abcdef";
    u32 len = size > str.length() ? str.length() : size;
    u64 out = 0;

    if (optionalHead && str.substr(off, 2) == "0x") {
        off += 2; len -= 2;
    }

    for (u32 i = 0; i < len; i++) {
        size_t j = hexMap.find(str[off + i]);
        if (j == std::string::npos) {
            printf("fromhex(): Malformed value: \"%s\"\n", str.substr(off, len).c_str());
            return 0;
        }
        out |= (j & 0xF) << (len - 1 - i) * 4;
    }
...
static std::string stringLower(const std::string& src) {
    std::string out = src;
...
static std::string stringUpper(const std::string& src) {
    std::string out = src;
    for (size_t i = 0; i < out.size(); i++) {
        if (out[i] >= 'a' && out[i] <= 'z')
            out[i] -= 0x20;
...
static std::string stringEscape(const std::string& src) {
    std::string out = src;
    size_t i;
    ...
    while ((i = out.find("\\r")) != std::string::npos)
        out = out.replace(i, 2, "\r");

    while ((i = out.find("\\t")) != std::string::npos)
        out = out.replace(i, 2, "\t");
    ...
    while ((i = out.find("\\x")) != std::string::npos)
        out = out.replace(i, 4, encode_utf8_unit((u32)fromhex(out, i + 2, 2)));

    while ((i = out.find("\\u")) != std::string::npos)
        out = out.replace(i, 6, encode_utf8_unit((u32)fromhex(out, i + 2, 4)));

    while ((i = out.find("\\U")) != std::string::npos)
        out = out.replace(i, 10, encode_utf8_unit((u32)fromhex(out, i + 2, 8)));

    i = 0;
    while ((i = out.find("\\\\", i + 1)) != std::string::npos)
        out = out.erase(i, 1);
...
static int cmd_make_smdh(SMDH& smdh, const std::string& icon, const std::string& smallIcon, const std::string& output) {
    ...
    if (noLargeIcon) {
        icon48Data = new u8[SMDH_LARGE_ICON_SIZE * SMDH_LARGE_ICON_SIZE * sizeof(u32)];
        memset(icon48Data, 0, SMDH_LARGE_ICON_SIZE * SMDH_LARGE_ICON_SIZE * sizeof(u32));
    } else {
        icon48Data = (u8*) load_image(icon.c_str(), SMDH_LARGE_ICON_SIZE, SMDH_LARGE_ICON_SIZE);
        if(icon48Data == NULL)
            return 1;
    }
...
static void cmd_print_version(){
    printf("bannertool v%d.%d.%d%s\n", VERSION_MAJOR, VERSION_MINOR, VERSION_MICRO, VERSION_SUFFIX);
}
...
static void cmd_print_info(const std::string& command) {
    if (command.compare("makebanner") == 0) {
        ...
        printf("  -i/--icon: Optional. PNG file to use as an icon.\n");
...
int cmd_process_command(int argc, char* argv[]) {
    ...
    if (strcmp(command, "makebanner") == 0) {
        ...
        if (icon.empty())
            printf("WARNING: No icon specified; using blank icon.\n");
        ...
        const std::string shortTitle = stringEscape(cmd_find_arg(args, "s", "shorttitle", ""));
        const std::string longTitle = stringEscape(cmd_find_arg(args, "l", "longtitle", ""));
        const std::string publisher = stringEscape(cmd_find_arg(args, "p", "publisher", ""));
        ...
        for (u32 i = 0; i < SMDH_NUM_LANGUAGE_SLOTS; i++) {
            ...
            if (i < SMDH_NUM_VALID_LANGUAGE_SLOTS) {
                currShortTitle = stringEscape(cmd_find_arg(args, shortTitleShortArgs[i], shortTitleLongArgs[i], shortTitle));
                currLongTitle = stringEscape(cmd_find_arg(args, longTitleShortArgs[i], longTitleLongArgs[i], longTitle));
                currPublisher = stringEscape(cmd_find_arg(args, publisherShortArgs[i], publisherLongArgs[i], publisher));
            }
...
//            cmd_missing_args(command);
//            return -1;
            printf("WARNING: Missing name fields.\n");
        }
        ...
        for (std::vector<std::string>::iterator it = regions.begin(); it != regions.end(); it++) {
            const std::string region = stringLower(*it);
        ...
        for(std::vector<std::string>::iterator it = flags.begin(); it != flags.end(); it++) {
            const std::string flag = stringLower(*it);
